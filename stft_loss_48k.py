...
def SNR(pred, target):
    return (20 * torch.log10(torch.norm(target, dim=-1).clamp(min=1e-8) / torch.norm(pred - target, dim=-1).clamp(min=1e-8))).mean()
...
#     estimate_source = trans(estimate_source)
    estimate_source = torch.stft(estimate_source_x, fft_size, hop_size, win_length, window_fn.to(estimate_source_x.device))
